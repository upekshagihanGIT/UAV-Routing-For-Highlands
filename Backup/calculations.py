...
def get_optimal_direction(bearing_direction, wind_direction):
    optimal_direction = None
    bearing_direction_sv = bearing_direction
    wind_direction_sv = wind_direction
    if (bearing_direction >= 270 and wind_direction <= 90) or (wind_direction >= 270 and bearing_direction <= 90):
        if bearing_direction >= 270:
            bearing_direction = 360 - bearing_direction
        if wind_direction >= 270:
            wind_direction = 360 - wind_direction

        optimal_direction = (bearing_direction + wind_direction) / 2 + max(bearing_direction_sv, wind_direction_sv)
        if optimal_direction >= 360:
            optimal_direction = optimal_direction - 360
        return optimal_direction

    optimal_direction = (abs(bearing_direction - wind_direction)) / 2 + min(bearing_direction_sv, wind_direction_sv)
    return optimal_direction